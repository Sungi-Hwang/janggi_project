...
#define LOGD_BITBOARD(...)                                                     \
  __android_log_print(ANDROID_LOG_DEBUG, "StockfishBitboard", __VA_ARGS__)
#else
#define LOGD_BITBOARD(...)
#endif
...
Magic *magics[] = {BishopMagics,       RookMagicsH,       RookMagicsV,
                   CannonMagicsH,      CannonMagicsV,     LameDabbabaMagics,
                   HorseMagics,        ElephantMagics,    JanggiElephantMagics,
                   CannonDiagMagics,   NightriderMagics,  GrasshopperMagicsH,
...
Bitboard lame_leaper_path(Direction d, Square s) {
  ...
  Direction df =
      (std::abs(d % NORTH) < NORTH / 2 ? d % NORTH : -(d % NORTH)) < 0 ? WEST
                                                                       : EAST;
  Square to = s + d;
  Bitboard b = 0;
  ...
  while (s != to) {
    int diff =
        std::abs(file_of(to) - file_of(s)) - std::abs(rank_of(to) - rank_of(s));
    if (diff > 0)
      s += df;
    else if (diff < 0)
      s += dr;
    ...
    for (auto modality : {MODALITY_QUIET, MODALITY_CAPTURE}) {
      for (bool initial : {false, true}) {
        ...
        auto &riderTypes = modality == MODALITY_CAPTURE
                               ? AttackRiderTypes[pt]
                               : MoveRiderTypes[initial][pt];
        riderTypes = NO_RIDER;
        for (auto const &[d, limit] : pi->steps[initial][modality]) {
          if (limit &&
              LameDabbabaDirections.find(d) != LameDabbabaDirections.end())
      ...
      for (Square s = SQ_A1; s <= SQ_MAX; ++s) {
        for (auto modality : {MODALITY_QUIET, MODALITY_CAPTURE}) {
          for (bool initial : {false, true}) {
            ...
            pseudo |=
                sliding_attack<RIDER>(pi->slider[initial][modality], s, 0, c);
            pseudo |= sliding_attack<HOPPER_RANGE>(
...
void Bitboards::init() {
  ...
  init_magics<HOPPER>(CannonTableV, CannonMagicsV, RookDirectionsV,
                      CannonMagicVInit);
  init_magics<LAME_LEAPER>(LameDabbabaTable, LameDabbabaMagics,
                           LameDabbabaDirections, LameDabbabaMagicInit);
  ...
  LOGD_BITBOARD("Initializing Rook Magics...");
  init_magics<RIDER>(RookTableH, RookMagicsH, RookDirectionsH);
  init_magics<RIDER>(RookTableV, RookMagicsV, RookDirectionsV);

  LOGD_BITBOARD("Skipping Bishop Magics (Unused for Janggi)...");
  // init_magics<RIDER>(BishopTable, BishopMagics, BishopDirections);

  LOGD_BITBOARD("Initializing Cannon Magics...");
  init_magics<HOPPER>(CannonTableH, CannonMagicsH, RookDirectionsH);
  init_magics<HOPPER>(CannonTableV, CannonMagicsV, RookDirectionsV);

  LOGD_BITBOARD("Initializing Lame Leaper Magics...");
  // init_magics<LAME_LEAPER>(LameDabbabaTable, LameDabbabaMagics,
  // LameDabbabaDirections);
  init_magics<LAME_LEAPER>(HorseTable, HorseMagics, HorseDirections);
  // init_magics<LAME_LEAPER>(ElephantTable, ElephantMagics,
  // ElephantDirections);
  init_magics<LAME_LEAPER>(JanggiElephantTable, JanggiElephantMagics,
                           JanggiElephantDirections);

  LOGD_BITBOARD("Skipping Remaining Magics (Unused for Janggi)...");
  // init_magics<HOPPER>(CannonDiagTable, CannonDiagMagics, BishopDirections);
  // init_magics<RIDER>(NightriderTable, NightriderMagics, HorseDirections);
  // init_magics<HOPPER>(GrasshopperTableH, GrasshopperMagicsH,
  // GrasshopperDirectionsH); init_magics<HOPPER>(GrasshopperTableV,
  // GrasshopperMagicsV, GrasshopperDirectionsV);
  // init_magics<HOPPER>(GrasshopperTableD, GrasshopperMagicsD,
  // GrasshopperDirectionsD);

  LOGD_BITBOARD("Magics initialization complete.");
#endif

  init_pieces();

  for (Square s1 = SQ_A1; s1 <= SQ_MAX; ++s1) {
    for (PieceType pt : {ROOK}) // Removed BISHOP to prevent crash
      for (Square s2 = SQ_A1; s2 <= SQ_MAX; ++s2) {
        if (PseudoAttacks[WHITE][pt][s1] & s2) {
          LineBB[s1][s2] =
              (attacks_bb(WHITE, pt, s1, 0) & attacks_bb(WHITE, pt, s2, 0)) |
              s1 | s2;
...
void init_magics(Bitboard table[], Magic magics[],
  ...
  for (Square s = SQ_A1; s <= SQ_MAX; ++s) {
    ...
    Magic &m = magics[s];
    // The mask for hoppers is unlimited distance, even if the hopper is limited
    // distance (e.g., grasshopper)
    m.mask = (MT == LAME_LEAPER
                  ? lame_leaper_path(directions, s)
                  : sliding_attack<MT == HOPPER ? HOPPER_RANGE : MT>(directions,
                                                                     s, 0)) &
             ~edges;
#ifdef LARGEBOARDS
    ...
    for (int i = 0; i < size;) {
      for (m.magic = 0; popcount((m.magic * m.mask) >> (SQUARE_NB - FILE_NB)) <
                        FILE_NB - 2;) {
#ifdef LARGEBOARDS
#ifdef PRECOMPUTED_MAGICS
...
#else
        m.magic =
            (rng.sparse_rand<Bitboard>() << 64) ^ rng.sparse_rand<Bitboard>();
#endif
#else
