...
template <Tracing T> template <Color Us> void Evaluation<T>::initialize() {
  ...
  attackedBy[Us][KING] =
      pos.count<KING>(Us) ? pos.attacks_from(Us, KING, ksq) : Bitboard(0);
  ...
  if (!pos.count<KING>(Us))
    ...
    Square s =
        make_square(std::clamp(file_of(ksq), FILE_B, File(pos.max_file() - 1)),
                    std::clamp(rank_of(ksq), RANK_2, Rank(pos.max_rank() - 1)));
    kingRing[Us] = attacks_bb<KING>(s) | s;
  }

  kingAttackersCount[Them] =
      popcount(kingRing[Us] & (pe->pawn_attacks(Them) |
                               shift<Down>(pos.pieces(Them, SHOGI_PAWN))));
...
Score Evaluation<T>::pieces(PieceType Pt) {
  ...
  while (b1) {
    ...
    if (pos.promoted_piece_type(Pt) != NO_PIECE_TYPE) {
      ...
      if (zone & (b | s))
        score += make_score(PieceValue[MG][pos.promoted_piece_type(Pt)] -
                                PieceValue[MG][Pt],
      ...
      score -=
          make_score(
              PieceValue[MG][Pt] - PieceValue[MG][pos.unpromoted_piece_on(s)],
    ...
    if (Pt == BISHOP || Pt == KNIGHT) {
      ...
      if (pos.count<KING>(Us))
        score -=
            KingProtector[Pt == BISHOP] * distance(pos.square<KING>(Us), s);
      ...
      if (Pt == BISHOP) {
        ...
        score -=
            BishopPawns[edge_distance(file_of(s), pos.max_file())] *
  ...
  if (pos.count_in_hand(Us, pt) > 0 && pt != KING) {
    ...
    Bitboard theirHalf =
        ...
        ~forward_ranks_bb(Them,
                          relative_rank(Them, Rank((pos.max_rank() - 1) / 2),
                                        pos.max_rank()));
    mobility[Us] +=
        DropMobility * popcount(b & theirHalf & ~attackedBy[Them][ALL_PIECES]);

    // Bonus for Kyoto shogi style drops of promoted pieces
    if (pos.promoted_piece_type(pt) != NO_PIECE_TYPE && pos.drop_promoted())
      score += make_score(std::max(PieceValue[MG][pos.promoted_piece_type(pt)] -
                                       PieceValue[MG][pt],
                                   VALUE_ZERO),
...
template <Tracing T> template <Color Us> Score Evaluation<T>::king() const {
  ...
  std::function<Bitboard(Color, PieceType)> get_attacks = [this](Color c,
                                                                 PieceType pt) {
  ...
  for (PieceSet ps = pos.piece_types(); ps;) {
    ...
    switch (pt) {
    case QUEEN:
      // Janggi has no Queen. Skip logic.
      break;
    case ROOK:
      // Standard Rook check logic
      knightChecks =
          attacks_bb(Us, pt, ksq, pos.pieces() ^ pos.pieces(Us, QUEEN)) &
      ...
      break;
    case BISHOP:
      // Janggi has no Bishop. Skip logic.
      break;
    case KNIGHT:
      knightChecks =
          attacks_bb(Us, pt, ksq, pos.pieces() ^ pos.pieces(Us, QUEEN)) &
    ...
    case PAWN:
      if (pos.piece_drops() && pos.count_in_hand(Them, pt) > 0) {
        pawnChecks = attacks_bb(Us, pt, ksq, pos.pieces()) & ~pos.pieces() &
                     pos.board_bb();
        if (pawnChecks & safe)
    ...
    case SHOGI_PAWN:
      if (pos.promoted_piece_type(pt)) {
        otherChecks =
            attacks_bb(Us, pos.promoted_piece_type(pt), ksq, pos.pieces()) &
  ...
  if (pos.check_counting())
    score +=
        make_score(0, mg_value(score) * 2 / (2 + pos.checks_remaining(Them)));

  if (pos.king_type() == WAZIR)
    score += make_score(0, mg_value(score) / 2);

  // For drop games, king danger is independent of game phase, but dependent on
  // material density
...
template <Tracing T> template <Color Us> Score Evaluation<T>::threats() const {
  ...
  if (pos.must_capture()) {
    ...
    score +=
        make_score(200, 220) * popcount(attackedBy[Them][ALL_PIECES] & moves &
  ...
  if (pos.extinction_value() == -VALUE_MATE) {
    ...
    for (PieceSet ps = pos.extinction_piece_types(); ps;) {
      ...
      if (pt == ALL_PIECES)
        continue;
      int denom = std::max(
          pos.count_with_hand(Them, pt) - pos.extinction_piece_count(), 1);
      // Explosion threats
      if (pos.blast_on_capture()) {
        int evasions = popcount(((attackedBy[Them][pt] & ~pos.pieces(Them)) |
                                 pos.pieces(Them, pt)) &
                                ~attackedBy[Us][ALL_PIECES]) *
                       denom;
  ...
  // Non-pawn enemies
  nonPawnEnemies =
      pos.pieces(Them) & ~pos.pieces(PAWN, SHOGI_PAWN) & ~pos.pieces(SOLDIER);
...
template <Tracing T> template <Color Us> Score Evaluation<T>::passed() const {
  ...
  while (b) {
    ...
    int r = std::max(RANK_8 -
                         std::max(relative_rank(Us, pos.promotion_square(Us, s),
                                                pos.max_rank()) -
  ...
  for (PieceSet ps = (pos.promotion_pawn_types(Us) & ~piece_set(PAWN)); ps;) {
    ...
    while (b2) {
      ...
      if (pos.promotion_square(Us, s) == SQ_NONE ||
        ...
        continue;
      int r = std::max(
          RANK_8 - std::max(relative_rank(Us, pos.promotion_square(Us, s),
                                          pos.max_rank()) -
                                relative_rank(Us, s, pos.max_rank()),
                            0),
          0);
  ...
  if (pt != NO_PIECE_TYPE) {
    ...
    while (b) {
      ...
      Square blockSq = s + Up;
      int d = 2 * std::max(relative_rank(Us, pos.promotion_square(Us, s),
                                         pos.max_rank()) -
                               relative_rank(Us, s, pos.max_rank()),
                           1);
...
template <Tracing T> template <Color Us> Score Evaluation<T>::space() const {
  ...
  if (pos.flag_region(Us))
    score +=
        make_score(200, 200) * popcount(behind & safe & pos.flag_region(Us));
...
template <Tracing T> template <Color Us> Score Evaluation<T>::variant() const {
  ...
  if (pos.flag_region(Us)) {
    ...
    Bitboard doubleBlocked =
        attackedBy2[Them] |
        (pos.pieces(Us, PAWN) &
         (shift<Down>(pos.pieces()) | attackedBy[Them][ALL_PIECES])) |
        (pos.pieces(Them) & pe->pawn_attacks(Them)) |
        (pawn_attacks_bb<Them>(pos.pieces(Them, PAWN) &
                               pe->pawn_attacks(Them)));
    Bitboard inaccessible =
        pos.pieces(Us, PAWN) & shift<Down>(pos.pieces(Them, PAWN));
    ...
    // This reflects that likely a move will be needed to block or capture the
    // attack. If all piece types are eligible, use the king path as a proxy for
    // distance.
    PieceType ptCtf =
        pos.flag_piece(Us) == ALL_PIECES ? KING : pos.flag_piece(Us);
    for (int dist = 0;
  ...
  if (pos.extinction_value() != VALUE_NONE) {
    for (PieceSet ps = pos.extinction_piece_types(); ps;) {
      ...
      if (pt != ALL_PIECES) {
        ...
        int denom =
            std::max(pos.count(Us, pt) - pos.extinction_piece_count(), 1);
        ...
        score +=
            make_score(pos.non_pawn_material(Us), pos.non_pawn_material(Us)) /
        ...
        for (File f = FILE_A; f <= pos.max_file(); ++f) {
          ...
          score -=
              make_score(80 - 10 * (edge_distance(f, pos.max_file()) % 2),
                         80 - 15 * (edge_distance(f, pos.max_file()) % 2)) *
              m / (1 + l * r);
        }
      } else if (pos.count<PAWN>(Them) == pos.count<ALL_PIECES>(Them)) {
        // Add a bonus according to how close we are to breaking through the
        // pawn wall
        int dist = 8;
  ...
  if (pos.flip_enclosed_pieces()) {
    ...
    if (pos.flip_enclosed_pieces() == REVERSI) {
      ...
      while (edgePieces) {
        Bitboard connectedEdge = attacks_bb(Us, ROOK, pop_lsb(edgePieces),
                                            ~(pos.pieces(Us) & edges)) &
                                 edges;
...
template <Tracing T> Value Evaluation<T>::winnable(Score score) const {
  ...
  // No initiative bonus for variants that do not require sufficient mating
  // material, e.g., extinction variants. This protects them from
  // misidentification as drawish.
  int complexity = 0;
...
Value fix_FRC(const Position &pos) {
  ...
  constexpr Bitboard Corners =
      Bitboard(1ULL) << SQ_A1 | Bitboard(1ULL) << SQ_H1 |
...
Value Eval::evaluate(const Position &pos) {
  ...
  if (!Eval::useNNUE || !pos.nnue_applicable())
    ...
    bool classical =
        psq * 5 > (750 + pos.non_pawn_material() / 64) * (5 + r50) && !pure;
...
std::string Eval::trace(Position &pos) {
  ...
  ss << std::showpoint << std::noshowpos << std::fixed << std::setprecision(2)
     ...
     << Term(SPACE) << "|    Variant | " << Term(VARIANT) << "|   Winnable | "
     << Term(WINNABLE)
     << "+------------+-------------+-------------+-------------+\n"
     << "|      Total | " << Term(TOTAL)
     << "+------------+-------------+-------------+-------------+\n";
