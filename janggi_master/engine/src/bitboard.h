...
inline Bitboard undo_move_board(Bitboard b, Move m) {
  return (from_sq(m) != SQ_NONE && (b & to_sq(m))) ? (b ^ to_sq(m)) | from_sq(m)
                                                   : b;
}
...
inline Bitboard attacks_bb(Square s, Bitboard occupied) {
  ...
  // case QUEEN : return attacks_bb<BISHOP>(s, occupied) | attacks_bb<ROOK>(s,
  // occupied);
  default:
    return PseudoAttacks[WHITE][Pt][s];
  }
...
/// lsb() and msb() return the least/most significant bit in a non-zero piece
/// set
...
/// pop_lsb() and pop_msb() find and clear the least/most significant bit in a
/// non-zero piece set
