...
namespace {
    ...
    Variant* janggi_variant() {
        ...
        // Fixed: Janggi uses "외상" layout (Chariot-Elephant-Horse)
        // Correct order: R(차) B(상) N(말) A(사) K(왕) A(사) N(말) B(상) R(차)
        v->startFen = "rbna1anbr/4k4/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/4K4/RBNA1ANBR w - - 0 1";
        v->mobilityRegion[WHITE][WAZIR] = v->mobilityRegion[WHITE][KING];
        v->mobilityRegion[BLACK][WAZIR] = v->mobilityRegion[BLACK][KING];
        // CRITICAL FIX: Janggi soldiers can move sideways from the start (unlike Xiangqi)
        // Set to RANK_1 (minimum) so relative_rank < RANK_1 is always false = all soldiers can move laterally
