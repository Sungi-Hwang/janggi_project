  ...
  for (Rank r = pos.max_rank(); r >= RANK_1; --r) {
    for (File f = FILE_A; f <= pos.max_file(); ++f)
      if (pos.state()->wallSquares & make_square(f, r))
        ...
        os << " |+"
           << pos.piece_to_char()[pos.unpromoted_piece_on(make_square(f, r))];
      else
...
void Position::init() {
  ...
  for (Color c : {WHITE, BLACK})
    for (PieceSet ps = CHESS_PIECES & ~piece_set(PAWN); ps;) {
      ...
      // Skip pieces that were disabled in Bitboards::init() to prevent crash
      ...
      for (Square s1 = SQ_A1; s1 <= SQ_MAX; ++s1)
        for (Square s2 = Square(s1 + 1); s2 <= SQ_MAX; ++s2)
          if ((type_of(pc) != PAWN) &&
            ...
            Key key =
                Zobrist::psq[pc][s1] ^ Zobrist::psq[pc][s2] ^ Zobrist::side;
            int i = H1(key);
            while (true) {
...
Position &Position::set(const Variant *v, const string &fenStr, bool isChess960,
  ...
  */
  ...
  while ((ss >> token) && !isspace(token)) {
    ...
    // Ignore pieces outside the board and wait for next / or [ to return to a
    // valid state
  ...
  ss >> token;
  sideToMove =
      (token != (sfen ? 'w' : 'b') ? WHITE : BLACK); // Invert colors for SFEN
  ss >> token;
  ...
  if (!isdigit(ss.peek()) && !sfen) {
    ...
    // standard, Shredder-FEN that uses the letters of the columns on which the
    // rooks began the game instead of KQkq and also X-FEN standard that, in
    // case of Chess960, if an inner rook is associated with the castling right,
    // the castling tag is replaced by the file letter of the involved rook, as
    // for the Shredder-FEN.
    while ((ss >> token) && !isspace(token)) {
      Square rsq;
      ...
      if (castling_enabled() && token == 'K')
        ...
        for (rsq =
                 make_square(var->castlingRookQueensideFile, castling_rank(c));
             (!(castling_rook_pieces(c) & type_of(piece_on(rsq))) ||
              color_of(piece_on(rsq)) != c) &&
      ...
      if (castling_enabled() && st->castlingKingSquare[c] == SQ_NONE) {
        ...
        // e.g., EAH means that an e-file king can castle with a- and h-file
        // rooks
        st->castlingKingSquare[c] =
            isChess960 && piece_on(rsq) == make_piece(c, castling_king_piece(c))
                ? rsq
      ...
      if (gating()) {
        ...
        // Do not set castling rights for gates unless there are no pieces in
        // hand, which means that the file is referring to a chess960 castling
        // right.
    ...
    if (gating() && castling_enabled())
      for (Color c : {WHITE, BLACK})
        if ((gates(c) & pieces(castling_king_piece(c))) &&
            !castling_rights(c) &&
    ...
    // Ignore if square is invalid or not on side to move relative rank 6.
      ...
      while (((ss >> col) && (col >= 'a' && col <= 'a' + max_file())) &&
        ...
        // b) there is an enemy pawn one or two (for triple steps) squares in
        // front of epSquare c) there is no (non-wall) piece on epSquare or
        // behind epSquare
        if ((var->enPassantRegion[sideToMove] & epSquare) &&
            (!var->fastAttacks ||
             (var->enPassantTypes[sideToMove] & ~piece_set(PAWN)) ||
             (pawn_attacks_bb(~sideToMove, epSquare) &
                  pieces(sideToMove, PAWN) &&
              ((pieces(~sideToMove, PAWN) &
                (epSquare + pawn_push(~sideToMove))) ||
               (pieces(~sideToMove, PAWN) &
...
void Position::set_check_info(StateInfo *si) const {
  ...
  si->shak =
      si->checkersBB & (byTypeBB[KNIGHT] | byTypeBB[ROOK] | byTypeBB[BERS]);
...
Position &Position::set(const string &code, Color c, StateInfo *si) {
  ...
  string sides[] = {
      code.substr(code.find('v') != string::npos ? code.find('v') + 1
                                                 : code.find('K', 1)), // Weak
...
string Position::fen(bool sfen, bool showPromoted, int countStarted,
                     std::string holdings, Bitboard fogArea) const {
    ...
    for (File f = FILE_A; f <= max_file(); ++f) {
      ...
      if (f <= max_file()) {
        if (empty(make_square(f, r)) || fogArea & make_square(f, r))
          ...
          // Set promoted pieces
          if (((captures_to_hand() && !drop_loop()) || two_boards() ||
               showPromoted) &&
  ...
  if (chess960 && can_castle(WHITE_CASTLING) &&
      popcount(pieces(WHITE, castling_king_piece(WHITE)) &
  ...
  if (chess960 && can_castle(BLACK_CASTLING) &&
      popcount(pieces(BLACK, castling_king_piece(BLACK)) &
...
Bitboard Position::slider_blockers(Bitboard sliders, Square s,
  ...
  if (var->fastAttacks) {
    snipers = ((attacks_bb<ROOK>(s) & pieces(c, QUEEN, ROOK, CHANCELLOR))
               // | (attacks_bb<BISHOP>(s) & pieces(c, QUEEN, BISHOP,
               // ARCHBISHOP)) // Janggi has no Bishop/Queen
               ) &
              sliders;
...
Bitboard Position::attackers_to(Square s, Bitboard occupied, Color c,
  ...
  if (diagonal_lines() & s) {
    ...
    //          ~janggiCannons) & pieces(c, JANGGI_CANNON);
...
Bitboard Position::checked_pseudo_royals(Color c) const {
  ...
  while (pseudoRoyals) {
    ...
    // skip if capturing this piece would blast any of the attacker's
    // pseudo-royal pieces
...
bool Position::legal(Move m) const {
  ...
  if (must_drop() && count_in_hand(us, var->mustDropType) > 0) {
    if (type_of(m) == DROP) {
      if (var->mustDropType != ALL_PIECES &&
          var->mustDropType != in_hand_piece_type(m))
        return false;
    } else if (checkers()) {
  ...
  if (var->extinctionPseudoRoyal) {
    ...
    if (type_of(m) == CASTLING) {
      ...
      if (st->pseudoRoyals & from)
        ...
        for (Square s = from; from != kto ? s != kto : s == from; s += step)
          if (!(blast_on_capture() && (attacks_bb<KING>(s) & st->pseudoRoyals &
                                       pieces(~sideToMove))) &&
    ...
    if (capture(m) && blast_on_capture())
      occupied &= ~((attacks_bb<KING>(kto) &
                     ((pieces(WHITE) | pieces(BLACK)) ^ pieces(PAWN))) |
                    kto);
    ...
    for (PieceSet ps = var->petrifyOnCaptureTypes & extinction_piece_types();
         ps;)
    ...
    if (var->dupleCheck) {
      Bitboard pseudoRoyalCandidates =
          st->pseudoRoyalCandidates & pieces(sideToMove);
  ...
  // mutuallyImmuneTypes (diplomacy in Atomar)-- In no-check Atomic, kings can
  // be beside each other, but in Atomar, this prevents them from actually
  // taking. Generalized to allow a custom set of pieces that can't capture a
...
bool Position::pseudo_legal(const Move m) const {
  ...
  // if walling, and walling is not optional, or they didn't move, do the
  // checks.
  if (walling() && (!var->wallOrMove || (from == to))) {
    ...
    if (!((board_bb() & ~((pieces() ^ from) | to)) & gating_square(m)))
      return false;
    if (!(var->wallingRegion[us] &
          gating_square(m)) ||          // putting a wall on disallowed square
  ...
  if (type_of(pc) == PAWN) {
    ...
    if (mandatory_pawn_promotion() && (promotion_zone(us) & to) &&
        !sittuyin_promotion())
      return false;
...
bool Position::gives_check(Move m) const {
  ...
  if (((type_of(m) != DROP && (blockers_for_king(~sideToMove) & from)) ||
      ...
      attackers_to(square<KING>(~sideToMove), occupied, sideToMove,
                   janggiCannons) &
          occupied)
    return true;
  ...
  if (more_than_one(diagonal_lines() & (to | square<KING>(~sideToMove)))) {
    ...
    PieceType diagType = pt == WAZIR ? FERS
                         : pt == SOLDIER
                             ? PAWN
                             // : pt == ROOK    ? BISHOP  <-- Causes crash due
                             // to uninitialized Bishop magics
                             : NO_PIECE_TYPE;
    if (diagType && (attacks_bb(sideToMove, diagType, to, occupied) &
      ...
      // Manual check for diagonal palace attack to avoid uninitialized
      // CannonDiagMagics
      if ((diagonal_lines() & to) && (diagonal_lines() & ksq) &&
          aligned(to, ksq, ksq)) {
        // Check if there is exactly one screen (hurdle) between cannon and king
        Bitboard between = between_bb(to, ksq);
        // exclude the king and cannon themselves from 'between' (between_bb
        // does this usually)
        if (popcount(between & occupied) == 1)
          return true;
      }
    }
  }
  ...
  case PIECE_PROMOTION:
    return attacks_bb(sideToMove, promoted_piece_type(type_of(moved_piece(m))),
                      to, pieces() ^ from) &
           square<KING>(~sideToMove);
...
void Position::do_move(Move m, StateInfo &newSt, bool givesCheck) {
  ...
  if (flip_enclosed_pieces() && !is_pass(m)) {
    ...
    if (flip_enclosed_pieces() == REVERSI) {
      Bitboard b = attacks_bb(us, QUEEN, to, ~pieces(~us)) &
  ...
  if (type_of(m) == DROP) {
    ...
    if (castling_dropped_piece() && rank_of(to) == castling_rank(us)) {
      if (type_of(pc) == castling_king_piece(us) &&
          file_of(to) == castling_king_file()) {
        st->castlingKingSquare[us] = to;
        Bitboard castling_rooks = pieces(us) & rank_bb(castling_rank(us)) &
  ...
  if (type_of(pc) == PAWN) {
    if (type_of(m) == PROMOTION || type_of(m) == PIECE_PROMOTION) {
      Piece promotion =
          make_piece(us, type_of(m) == PROMOTION ? promotion_type(m)
  ...
  else if (type_of(m) != DROP && ((PseudoMoves[1][us][type_of(pc)][from] &
                                   ~PseudoMoves[0][us][type_of(pc)][from]) &
                                  to)) {
  ...
  if (captured && (blast_on_capture() || var->petrifyOnCaptureTypes)) {
    ...
    while (blast) {
      ...
      // unpromotedBoard, demote promoted pieces, but keep promoted pawns as
      // promoted, and store demotion/promotion bitboards to disambiguate the
      // piece state
      bool capturedPromoted = is_promoted(bsq);
      Piece unpromotedCaptured = unpromoted_piece_on(bsq);
      st->unpromotedBycatch[bsq] =
          unpromotedCaptured ? unpromotedCaptured : bpc;
  ...
  if (counting_rule()) {
    ...
    if ((!st->countingLimit || ((captured || type_of(m) == PROMOTION) &&
      ...
      st->countingLimit = 2 * count_limit(sideToMove);
      st->countingPly =
          counting_rule() == ASEAN_COUNTING || count<ALL_PIECES>(sideToMove) > 1
              ? 0
              : 2 * count<ALL_PIECES>();
    }
  }
...
void Position::undo_move(Move m) {
  ...
  if (type_of(m) == PROMOTION) {
    ...
    assert(type_of(st->promotionPawn) == main_promotion_pawn_type(us) ||
           !captures_to_hand());
  ...
  if (type_of(m) == CASTLING) {
    ...
    if (type_of(m) == DROP)
      undrop_piece(make_piece(us, in_hand_piece_type(m)),
                   to); // Remove the dropped piece
    else
      ...
      if (captures_to_hand())
        remove_from_hand(!drop_loop() && st->capturedpromoted
                             ? (st->unpromotedCapturedPiece
                                    ? ~st->unpromotedCapturedPiece
                                    : make_piece(~color_of(st->capturedPiece),
                                                 main_promotion_pawn_type(us)))
                             : ~st->capturedPiece);
    }
  }
...
bool Position::see_ge(Move m, Value threshold) const {
  ...
  while (true) {
    ...
    if (walling_rule() == DUCK)
      stmAttackers &=
          attacks_bb<KING>(to) | ~(pieces(BISHOP, ROOK) | pieces(QUEEN));
...
/// Position::is_optional_game_end() tests whether the position may end the game
...
bool Position::is_optional_game_end(Value &result, int ply,
  ...
  if (n_fold_rule()) {
    ...
    if (end >= 4) {
      ...
      int cnt = 0;
      bool perpetualThem =
          var->perpetualCheckIllegal && st->checkersBB && stp->checkersBB;
      bool perpetualUs = var->perpetualCheckIllegal &&
                         st->previous->checkersBB && stp->previous->checkersBB;
      Bitboard chaseThem =
          undo_move_board(st->chased, st->previous->move) & stp->chased;
      Bitboard chaseUs = undo_move_board(st->previous->chased, stp->move) &
                         stp->previous->chased;
      int moveRepetition =
          var->moveRepetitionIllegal && type_of(st->move) == NORMAL &&
                  !st->previous->checkersBB && !stp->previous->checkersBB &&
                  (board_bb(~side_to_move(),
                            type_of(piece_on(to_sq(st->move)))) &
      ...
      for (int i = 4; i <= end; i += 2) {
        ...
        if (stp->key == st->key &&
            ++cnt + 1 >= (ply > i && !moveRepetition && !chaseUs &&
                                  !chaseThem && !perpetualUs && !perpetualThem
                              ? 2
                              : n_fold_rule())) {
...
bool Position::is_immediate_game_end(Value &result, int ply) const {
  ...
  // Extinction
  // Extinction does not apply for pseudo-royal pieces, because they can not be
  // captured
    ...
    for (Color c : {~sideToMove, sideToMove})
      for (PieceSet ps = extinction_piece_types(); ps;) {
        ...
        if (count_with_hand(c, pt) <= var->extinctionPieceCount &&
          ...
          result =
              c == sideToMove ? extinction_value(ply) : -extinction_value(ply);
          return true;
        }
      }
  }
  ...
  if (flag_move() && flag_reached(sideToMove)) {
    result =
        sideToMove == WHITE && flag_reached(BLACK) ? VALUE_DRAW : mate_in(ply);
    return true;
  }
  // A direct flag win is possible if the opponent does not get an extra flag
  // move or we can detect early for kings that they won't be able to reach the
  // flag region Note: This condition has to be after the above, since both
  // might be true e.g. in racing kings.
  if ((!flag_move() ||
       flag_piece(sideToMove) ==
           KING) // we can do early win detection only for the king
    ...
    if (flag_move() && sideToMove == BLACK && !checkers() &&
      ...
      for (const auto &m : MoveList<NON_EVASIONS>(*this))
        if (type_of(moved_piece(m)) == KING &&
            (flag_region(sideToMove) & to_sq(m)) && legal(m)) {
          gameEnd = false;
          break;
        }
    }
  ...
  if (var->castlingWins) {
    if (st->pliesFromNull > 0 && type_of(st->move) == CASTLING) {
      ...
      CastlingRights justCastled =
          ~sideToMove &
          ((from_sq(st->move) < to_sq(st->move)) ? KING_SIDE : QUEEN_SIDE);
      if (var->castlingWins & justCastled) {
        result = mated_in(ply);
        return true;
      }
    }
    // We check the opponent side first, because a rook capturing a rook could
    // remove both sides castling rights, which should likely be seen as losing,
    // analogous to extinction rules.
    for (Color c : {~sideToMove, sideToMove})
      if ((c & var->castlingWins) &&
          !(c & var->castlingWins & st->castlingRights)) {
        // player permanently losing castling rights. either through moving a
        // castling piece, or having their rook captured.
  ...
  if ((var->connectRegion1[~sideToMove] & connectPieces) &&
    ...
    while (true) {
      ...
      for (Direction d : var->connectDirections) {
        newBitboard |= shift(d, current | newBitboard) &
                       connectPieces; // the "| newBitboard" here probably saves
                                      // a few loops
      }
  ...
  if (connect_nxn()) {
    ...
    for (int i = 1; i < connect_nxn() && connectors; i++)
      connectors &= shift<SOUTH>(connectors) & shift<EAST>(connectors) &
                    shift<SOUTH_EAST>(connectors);
    if (connectors) {
      result = convert_mate_value(-var->connectValue, ply);
      return true;
    }
...
Bitboard Position::chased() const {
  ...
  if (var->flyingGeneral) {
    Bitboard kingFilePieces =
        file_bb(file_of(square<KING>(~sideToMove))) & pieces(sideToMove);
  ...
  auto addChased = [&](Square attackerSq, PieceType attackerType,
    ...
    if (attacks & ~b) {
      ...
      while (attacks) {
        ...
        Bitboard roots =
            attackers_to(s, pieces() ^ attackerSq, sideToMove) & ~pins;
        if (!roots ||
  ...
  if (movedPiece != KING && movedPiece != SOLDIER) {
    Bitboard directAttacks =
        attacks_from(~sideToMove, movedPiece, to) & pieces(sideToMove);
    // Only new attacks count. This avoids expensive comparison of previous and
    // new attacks.
...
bool Position::pos_is_ok() const {
  ...
  if ((sideToMove != WHITE && sideToMove != BLACK) ||
      ...
      (ep_squares() &
       ~(var->enPassantRegion[WHITE] | var->enPassantRegion[BLACK])))
  ...
  for (Color c : {WHITE, BLACK})
    for (CastlingRights cr : {c & KING_SIDE, c & QUEEN_SIDE}) {
      ...
      if (!(castling_rook_pieces(c) &
            type_of(piece_on(castlingRookSquare[cr]))) ||
